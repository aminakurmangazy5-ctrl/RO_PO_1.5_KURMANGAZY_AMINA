...
DROP TABLE IF EXISTS appointment_procedures CASCADE;
...
--Add cancelled_reason column to track reasons of cancellations
...
ALTER TABLE medical_history ADD CONSTRAINT uq_patient_condition UNIQUE (patient_id, condition_name);

--Set default status
ALTER TABLE appointments ALTER COLUMN status SET DEFAULT 'Scheduled';

-- Rename (и сразу исправляем UPDATE ниже)
ALTER TABLE procedures RENAME COLUMN base_price TO standard_price;
...
('Albina', 'Marat', 'Periodontist', 'KZ-PER-006', '+77019997766', 'albina.marat@dental.kz');


INSERT INTO procedures (procedure_code, procedure_name, description, base_price, category) VALUES
...
SET price_at_time = p.base_price * 1.12
FROM procedures p
...
GRANT INSERT, UPDATE, DELETE ON appointments, invoices, payments TO dental_writer;

--Revoke update on dentists - only admin can modify dentist data
